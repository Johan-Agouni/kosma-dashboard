...
import { getStatsApi, getRevenueApi, getTopProductsApi, getRecentOrdersApi } from '../api/dashboard.api';
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setRevenue(revenueRes.data.data.map(r => ({
            ...
            <div className={styles.content}>
                ...
                <div className="grid-4">
                    <StatsCard
                        ...
                        icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} width={24} height={24}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
                    />
                    <StatsCard
                        title="Commandes"
                        value={stats?.orders?.total || 0}
                        color="var(--color-info)"
                        icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} width={24} height={24}><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 10.5V6a3.75 3.75 0 10-7.5 0v4.5m11.356-1.993l1.263 12c.07.665-.45 1.243-1.119 1.243H4.25a1.125 1.125 0 01-1.12-1.243l1.264-12A1.125 1.125 0 015.513 7.5h12.974c.576 0 1.059.435 1.119 1.007z" /></svg>}
                    />
                    <StatsCard
                        title="Produits actifs"
                        value={stats?.products?.active || 0}
                        color="var(--color-accent)"
                        icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} width={24} height={24}><path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>}
                    />
                    <StatsCard
                        title="Stock bas"
                        value={stats?.products?.lowStock || 0}
                        color="var(--color-danger)"
                        icon={<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} width={24} height={24}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126z" /></svg>}
                    />
                </div>
                    ...
                    <div className="card">
                        ...
                        <ResponsiveContainer width="100%" height={280}>
                            <LineChart data={revenue}>
                                ...
                                />
                                <Line type="monotone" dataKey="revenue" stroke="var(--color-accent)" strokeWidth={2} dot={{ r: 4 }} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    <div className="card">
                        ...
                        <ResponsiveContainer width="100%" height={280}>
                            <BarChart data={topProducts} layout="vertical">
                                ...
                                <YAxis dataKey="name" type="category" width={120} stroke="var(--color-text-muted)" fontSize={11} />
                                <Tooltip contentStyle={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 8 }} />
                                <Bar dataKey="totalRevenue" fill="var(--color-accent)" radius={[0, 4, 4, 0]} />
                            </BarChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                        ...
                        <tbody>
                            {recentOrders.map(order => (
                                <tr key={order._id}>
                                    ...
                                    <td className={styles.dateCell}>{formatRelative(order.createdAt)}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
